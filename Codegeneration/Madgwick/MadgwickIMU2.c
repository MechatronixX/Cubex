...
#define betaDef2		0.1f		// 2 * proportional gain

//---------------------------------------------------------------------------------------------------
// Variable definitions

volatile float beta2 = betaDef2;								// 2 * proportional gain (Kp)
...
void MadgwickAHRSupdateIMU2(float in1, float in2, float in3, float in4, float in5, float in6, float *out1, float *out2, float *out3, float *out4){
	...
	if(!((in1 == 0.0f) && (in2 == 0.0f) && (in3 == 0.0f))) {
		...
		recipNorm = invSqrt2(in1 * in1 + in2 * in2 + in3 * in3);
		in1 *= recipNorm;
		in2 *= recipNorm;
		in3 *= recipNorm;   
	...
	recipNorm = invSqrt2(q02 * q02 + q12 * q12 + q22 * q22 + q32 * q32);
	q02 *= recipNorm;
	q12 *= recipNorm;
	q22 *= recipNorm;
	q32 *= recipNorm;
